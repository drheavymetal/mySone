...
/// Pull the user's Last.fm history into the local stats DB.
/// Mirror of the ListenBrainz command — relies on the embedded API key
/// + the connected username, no session needed for public profiles.
/// Streams progress via the `import-lastfm-progress` event.
#[tauri::command(rename_all = "camelCase")]
pub async fn import_lastfm_history(
    state: State<'_, AppState>,
    since_unix: Option<i64>,
) -> Result<crate::scrobble::ImportResult, SoneError> {
    log::info!("[import_lastfm_history]: since_unix={:?}", since_unix);
    state
        .scrobble_manager
        .import_lastfm_history(since_unix)
        .await
}
